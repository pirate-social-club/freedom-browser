  ...
  const ipcRenderer =
    ...
    createIpcRendererMock({
      ...
      invokeResponses: {
        ...
        [IPC.ANYONE_GET_STATUS]: { state: 'off', connected: false, error: null },
...
describe('preload', () => {
  ...
  test('exposes the preload bridges and routes direct wrappers to ipcRenderer', async () => {
    ...
    const invokeCases = [
      ...
      [exposures.electronAPI, 'resolveSpace', ['@space'], IPC.SPACES_RESOLVE, [{ handle: '@space' }]],
      ...
      [exposures.anyone, 'start', [], IPC.ANYONE_START, []],
      [exposures.anyone, 'stop', [], IPC.ANYONE_STOP, []],
      [exposures.anyone, 'getStatus', [], IPC.ANYONE_GET_STATUS, []],
      [exposures.dvpn, 'start', [], IPC.DVPN_START, []],
      [exposures.dvpn, 'stop', [], IPC.DVPN_STOP, []],
      [exposures.dvpn, 'getStatus', [], IPC.DVPN_GET_STATUS, []],
      [exposures.dvpn, 'getBalance', [], IPC.DVPN_GET_BALANCE, []],
      [exposures.dvpn, 'createWallet', [], IPC.DVPN_CREATE_WALLET, []],
      [exposures.dvpn, 'getWalletAddress', [], IPC.DVPN_GET_WALLET_ADDRESS, []],
      [exposures.dvpn, 'generateQR', ['sent1address', { width: 192 }], IPC.DVPN_GENERATE_QR, ['sent1address', { width: 192 }]],
      [exposures.dvpn, 'exportMnemonic', [], IPC.DVPN_EXPORT_MNEMONIC, []],
      [exposures.dvpn, 'checkPrerequisites', [], IPC.DVPN_CHECK_PREREQUISITES, []],
  ...
  test('status update wrappers subscribe, fetch current state immediately, and clean up', async () => {
    ...
    const statusCases = [
      ...
      [exposures.anyone, IPC.ANYONE_STATUS_UPDATE, IPC.ANYONE_GET_STATUS, { state: 'off', connected: false, error: null }, { state: 'connected' }],
    ];
